...
import {
  createStudent,
  getStudent,
} from '../../actions/student'; // Server Actions
import { recordVisit } from '../../actions/scans'; // Server Action
...
export default function StudentRegistration({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!existingStudent) {
        // Create new student (This action handles Student creation, Scan creation, and Org update atomically)
        await createStudent(studentId, email, fullName.trim(), organizationId, organizationName, boothNumber);
      } else {
        // Existing student - just record a visit
        await recordVisit(
            studentId, 
            email, 
            existingStudent.program || 'Computer Science', 
            organizationId, 
            organizationName, 
            boothNumber
        );
      ...
      if (msg.includes('Already visited')) {
         setError('You have already visited this stall.');
         toast.error('Already visited');
      } else {
